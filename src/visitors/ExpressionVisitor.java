...
//		symbol.setScope();
	...
	public Expression visitReferenceExpression(ReferenceExpressionContext ctx) {
		...
		ReferenceExpression referenceExpression = new ReferenceExpression(ctx.getChild(0).getText());
...
//		System.out.println("name Var  : " + ctx.getChild(0).getText());
		...
		symbolReference.setName(ctx.getChild(0).getText());
		symbolReference.setScope(currentScope);

		referenceExpression.setSymbol(symbolReference);
		currentScope.addSymbol(symbolReference.getName(),symbolReference);
